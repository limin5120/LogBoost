...
from logboost.dataGenerator.sample import sliding_window, session_window
...
class Trainer():
    def __init__(self, model, options):
        ...
        self.featuretype = [
            self.sequentials, self.quantitatives, self.semantics
        ]
        ...
        train_dataset = log_to_tensor(logs=train_logs,
                                      labels=train_labels,
                                      seq=self.sequentials,
                                      quan=self.quantitatives,
                                      sem=self.semantics)
        valid_dataset = log_to_tensor(logs=val_logs,
                                      labels=val_labels,
                                      seq=self.sequentials,
                                      quan=self.quantitatives,
                                      sem=self.semantics)

        del train_logs, train_labels, val_logs, val_labels
        gc.collect()
        ...
        self.log = {
            "train":
            {key: []
             for key in ["epoch", "lr", "time", "loss", "batch_time"]},
            "valid": {key: []
    ...
    def train(self, epoch):
        ...
        print("Starting epoch: %d | time: train | ⏰: %s | Learning rate: %f" %
              (epoch, start, lr))
        ...
        for i, (log, label) in enumerate(tbar):
            ...
            loss = criterion(output, label.long().to(self.device))
        ...
        self.log['train']['batch_time'].append(time.time() - begin)
    ...
    def start_train(self):
        ...
        for epoch in range(self.start_epoch, self.max_epoch):
            ...
            self.train(epoch)
            if epoch >= self.max_epoch // 2 and epoch % self.save_interval == 0:
                self.valid(epoch)
                self.save_checkpoint(epoch,
                                     save_optimizer=True,
